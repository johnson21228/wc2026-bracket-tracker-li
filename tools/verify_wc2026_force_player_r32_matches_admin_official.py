...
if missing:
    print("Force-player-R32-matches-Admin verification superseded by site-owned official truth runtime source:")
    for rel in missing:
        print(f"- missing {rel}")
    raise SystemExit(1)
...
if "data/current/official_truth.json" not in runtime or "player picks remain Supabase-backed" not in runtime:
    print("Force-player-R32-matches-Admin verification superseded by site-owned official truth runtime source: replacement runtime verifier is not protecting site-owned official truth source")
    raise SystemExit(1)

print("OK: Force-player-R32-matches-Admin verifier superseded by site-owned official truth runtime source.")
