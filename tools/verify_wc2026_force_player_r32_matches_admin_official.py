...
require("function selectedTeam(slotId)" in model and "return getTeam(picks[slotId]);" in model, "R32 display must render from player picks after Supabase Admin R32 hydration.")
require("hydrateOnlySupabaseAdminR32IntoPlayerPicks" in model and "stripR32OccupantsFromPlayerPicks(sourcePicks)" in model, "Loaded player/local R32 occupant values must be stripped before Supabase Admin R32 hydration.")
