...
if 'data-presentation-style="material-linework-prominent-r32"' not in svg_text:
...
for p in paths:
    ...
    if stroke != '#aa9691': raise SystemExit(f'Connector path must use #AA9691 stroke, found {stroke}')
...
for r in rects:
    ...
    if stroke != '#aa9691': raise SystemExit(f'Slot outline must use #AA9691 stroke, found {stroke}')
...
if manifest.get('presentationStyle')!='material_linework_prominent_r32': raise SystemExit('Manifest missing material_linework_prominent_r32 style')
policy=manifest.get('presentationPolicy',{})
if (policy.get('connectorStroke') or '').lower()!='#aa9691': raise SystemExit('Manifest connectorStroke must be #AA9691')
...
for marker in ['uniform_pick_card_gameboard.svg','uniform-svg-r32-manifest','material-linework-prominent-r32','applyUniformSvgR32ManifestGeometryToGame1SlotRules','class="pickLayer" id="pickLayer"','class="hitLayer" id="hitLayer"','game1_pub_options_background.jpeg']:
...
print('WC2026 Game 1 SVG linework prominence checks passed.')
